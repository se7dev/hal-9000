...
use crate::util::regex::{ENDGIVEAWAY, ENDVOTE, ENTERGIVEAWAY, PING, STARTGIVEAWAY, STARTVOTE, VOTE};
...
impl MainController {
    ...
    pub fn listen(&mut self) {
        ...
        if let Err(e) = recv_client.identify() { panic!("Error in auth {:?}", e) }
        ...
        recv_client.for_each_incoming(|irc_msg| {
            ...
            match &irc_msg.command {
                &Command::PRIVMSG(ref channel, ref message) => {
                    ...
                    if let Err(e) = block_on(self.db_connector.write_logs(message.clone().as_str())) {
